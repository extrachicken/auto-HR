...
def generate_jobdesc_questions_json(job_description: str, prompt: str, openai_api_key: str, must_haves: str):
    ...
    try:
        ...
        # --- Новый блок: переводим вопросы на язык job_description, если нужно ---
        target_language = detect_language(job_description)
        logger.info(f"Определён язык job_description: {target_language}")
        for field in fields:
            if 'title' in field:
                # Переводим только если язык отличается
                field['title'] = translate_text(field['title'], target_language)
            # Переводим варианты ответов, если есть
            if field.get('type') == 'multiple_choice' and 'properties' in field and 'choices' in field['properties']:
                for choice in field['properties']['choices']:
                    if 'label' in choice:
                        choice['label'] = translate_text(choice['label'], target_language)
